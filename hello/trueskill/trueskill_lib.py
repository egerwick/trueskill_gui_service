...
def games_in_trueskill(f_games, f_mu, f_tau, track_mu_over_time, player_combination=None):
    ...
    list_players = []
    env = TrueSkill(mu=f_mu, tau=f_tau, beta=4.167, draw_probability=0.05)
    env.make_as_global()
    average_rating = 25
    game_number = 0
...
def output_player_results(f_list_players, f_min_number_games):
    ...
    for players in f_list_players:
        total_games = total_games + float(players.ngame)
        if(players.ngame > f_min_number_games):
            print players.name, players.position, players.crawl, players.ngame, float(players.won) / float(players.ngame), float(players.goals) / float(players.ngame), players.rating.mu, players.rating.sigma, len(players.rating_history)
    print
    print total_games / 4
